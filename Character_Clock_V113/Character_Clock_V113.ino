...
The FM-module can be used to read the RDS-time from radio station or to play FM-radio station from within the clock
 ...
 changes : V113 Added optin W in menu; Test LDR every second       
 */
// ===============================================================================================================================
char VERSION[] = "Character_Clock_V113";
//--------------------------------------------
...
const char menu[][42] PROGMEM =  {
 ...
 "E Suppress Het Is Was after 10 secs",
 "C Reset settings",
...
// CLOCK Version info
//--------------------------------------------
// Storing strings in PROGMEM saves a lot of 
...
/*
// sprintf(sptext,"LDR average bits measured per hour"); 
...
{ 
 ...
 PrintLine(40);
   sprintf(sptext,"%s",strcpy_P(sptext, SWVER[0])); Tekstprintln(sptext);
 ...
 sprintf(sptext,"%s%3d bits Max: %3d%%",strcpy_P(sptext, SWVER[1]),Mem.LowerBrightness, Mem.LightReducer); Tekstprintln(sptext);
 sprintf(sptext,"%s%4d bits Max: %3d bits",strcpy_P(sptext, SWVER[2]),MinPhotocell, MaxPhotocell);Tekstprintln(sptext); 
 sprintf(sptext,"%s%02d - %02d",strcpy_P(sptext,SWVER[3]),Mem.TurnOffLEDsAtHH,Mem.TurnOnLEDsAtHH);Tekstprintln(sptext);
                                   #ifdef DCFMOD   
 sprintf(sptext,"%s%s  Signal:%3d%%",strcpy_P(sptext,SWVER[4]),UseDCF ? "On" : "Off",DCF_signal); Tekstprintln(sptext);
                                   #endif // DCFMOD   
 sprintf(sptext,"%s%s",strcpy_P(sptext,SWVER[5]),VERSION);                                        Tekstprintln(sptext); 
 GetTijd(1);  
 PrintLine(40);
}
 ...
 if (DCFtime!=0)
  {
   ...
   setTime(DCFtime);
   Inow = RTCklok.now(); 
   int diff = 1000+Inow.unixtime() - DCFtime;           // time is a uint32_t. -1 gives 4294967295 the maximum value for a 32-bit unsigned integer     
   diff -=1000;
   if(abs(diff) > 2)                                                  // If the time differs more than a second -> update
      {
       RTCklok.adjust(DCFtime);
       Tekstprint("DCF: Time updated ->"); 
       Serial.println(diff);
       Print_RTC_tijd();
      }
   else 
      {
       Tekstprint("DCF: Time not updated. Sec dif: ");
       Serial.println(abs(Inow.unixtime() - DCFtime));
      }
  }
 ...
 if (KeypadString.length()>5)                                       // If six numbers are entered rework this to a time hhmmss
   ...
   if(KeypadString=="999999")
     { 
      KeypadString = "";   
      Reset();
      Serial.println(F("Clock Settings reset"));   
     }
    else 
     {    
...
// or nothing when the DS3231 is not connected
// We use this Wire routine to avoid an error when 
// no DS3231 is present and the internal clock is used 
//--------------------------------------------
              ...
              if(InputString.length() == 3)
               {
                ...
                Tekstprintln("EEPROM data erased"); 
                setup();
               }
               break;
    case 'D':
    ...
    case 'l':
             if (InputString.length() < 5)
               {      
                ...
                sprintf(sptext,"Lower brightness now: %d bits",Mem.LowerBrightness);
                EEPROM.put(0,Mem);                                    // Update EEPROM     
               }
             break;  
    ...
    case 'm':
             ...
             if (InputString.length() < 5)
               {    
                ...
                sprintf(sptext,"Max brightness now: %d%%",Mem.LightReducer);
                EEPROM.put(0,Mem);                                                    // Update EEPROM     
               }
              break;
    case 'N':
